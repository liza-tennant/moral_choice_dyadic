...
class ExplorationPolicy:   
    def __init__(self, state_index, policy_type, QV, RNs): #myVars
    ...
    def use_policy(self, iteration, total_iterations, eps0, epsdecay):
        ...
        if self.policy_type == 'eps_greedy': 
            ...
            if prob <= eps: 
                ...
                #myStr = f'Q_values_player{self.player_index}'
                #Q_values_variable = self.myVars[myStr]
                
                #move optimally based on current Q-value estimates, if they are not empty 
                #if not np.any(Q_values_variable[self.state]): #if Q-values for this state are ampty (all 0) 
                if not np.any(self.QV[self.state_index]): #if empty
