...
class Game_for_learning:
    ...
    It takes in sgtates, allows each player to take an action (in the 'step' function) 
    ...
    def step(self, state_player1, state_player2, iteration, global_history, num_iter, RNG):
        ...
        #NOTE there is also a 5ht element in the RNG sequence - this is used to generate the intial state within the main script
    ...
    def step_static(self, state_player1, state_player2, iteration, global_history, RNG):
        ...
        action_player1 = self.player1.make_own_move(state=state_player1, playerx_RN_4=player1_RN_4)
        action_player2 = self.player2.make_own_move(state=state_player2, playerx_RN_4=player2_RN_4)

        #action_player1 = self.player1.make_own_move(self, player_index=1, myVars=myVars, global_history=global_history)
        #action_player2 = self.player2.make_own_move(self, player_index=2, myVars=myVars, global_history=global_history)
        #make_exploratory_move(state = state_player2, player_index=2, myVars=myVars)
    ...
    def step_mixed(self, state_player1, state_player2, iteration, global_history, num_iter, RNG): #myVars
        ...
        #save the key information as next_state for each agent #NOTE we will record state with opponent's move first, then own movement
        next_state_player1 = (action_player2, action_player1)
        ...
        return int(action_player1), next_state_player1, next_state_player2, reward_learning_player1 #int(next_state_player1), int(next_state_player2)
